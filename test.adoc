...
image:https://github.com/DinTamari/graphgist/blob/master/foodpic.png
...
For demonstration purposes: X = "Tomato", naturally since its now or never.
...
== USECASE4: Given ingredient X, find cocoingredients with have mutual coingredients with X. What.
...
WHERE NOT (tom)-[:IS_INGREDIENT]->(m2)
RETURN cocoIngredients.name AS Possibilities, count(*) AS Strength ORDER BY Strength DESC
----
